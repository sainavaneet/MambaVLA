...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        assert len_embd % n_head == 0
        # key, query, value projections for all heads, but in a batch
        self.key = nn.Linear(len_embd, len_embd)
        self.query = nn.Linear(len_embd, len_embd)
        self.value = nn.Linear(len_embd, len_embd)
        # output projection
        self.c_proj = nn.Linear(len_embd, len_embd, bias=bias)
        # regularization
        self.attn_dropout = nn.Dropout(attn_pdrop)
        self.resid_dropout = nn.Dropout(resid_pdrop)
        self.n_head = n_head
        self.len_embd = len_embd
        self.causal = causal
    ...
    def forward(self, x, context=None, custom_attn_mask=None):
        B, T, C = x.size() # batch size, sequence length, embedding dimensionality (len_embd)
...
class MLP(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.c_fc    = nn.Linear(len_embd, 4 * len_embd, bias=bias)
        self.gelu    = nn.GELU()
        self.c_proj  = nn.Linear(4 * len_embd, len_embd, bias=bias)
...
class Block(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.ln_1 = LayerNorm(len_embd, bias=bias)
        self.attn = Attention(len_embd, n_heads, attn_pdrop, resid_pdrop, block_size, causal, bias, use_rot_embed, rotary_xpos)
        self.use_cross_attention = use_cross_attention
        if self.use_cross_attention:
            self.cross_att = Attention(len_embd, n_heads, attn_pdrop, resid_pdrop, block_size, causal, bias, use_rot_embed, rotary_xpos)
            self.ln3 = nn.LayerNorm(len_embd)
        self.ln_2 = LayerNorm(len_embd, bias=bias)
        self.mlp = MLP(len_embd, bias, mlp_pdrop)
...
class ConditionedBlock(Block):
    ...
    def __init__(
        ...
        super().__init__(len_embd, n_heads, attn_pdrop, resid_pdrop, mlp_pdrop, block_size, causal,
