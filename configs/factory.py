...
def _get_class_from_target(target: str) -> Type:
    ...
    elif target == "RoboMani.RoboMani" or target == "RoboMani.RoboMani":
        # Import via package __init__ to keep a stable public API
        from RoboMani import RoboMani
        return RoboMani
    elif target == "RoboMani.Trainer":
        from RoboMani.main import Trainer
        return Trainer
    elif target == "RoboMani.benchmark.libero.libero_sim.LiberoSim":
        from RoboMani.benchmark.libero.libero_sim import MultiTaskSim
        return MultiTaskSim
    elif target == "RoboMani.ActionFLowMatching":
        from RoboMani import ActionFLowMatching
        return ActionFLowMatching
    elif target == "RoboMani.RoboManiPolicy":
        from RoboMani import RoboManiPolicy
        return RoboManiPolicy
    elif target == "RoboMani.MambaModel":
        from RoboMani import MambaModel
        return MambaModel
    elif target == "RoboMani.CLIPImgEncoder":
        from RoboMani import CLIPImgEncoder
        return CLIPImgEncoder
    elif target == "RoboMani.LangClip":
        from RoboMani import LangClip
        return LangClip
    elif target == "RoboMani.MultiImageObsEncoder":
        from RoboMani import MultiImageObsEncoder
        return MultiImageObsEncoder
    elif target == "RoboMani.ResNetEncoder":
        from RoboMani import ResNetEncoder
        return ResNetEncoder
    elif target == "RoboMani.benchmark.libero.libero_dataset.LiberoDataset":
        from RoboMani.benchmark.libero.libero_dataset import LiberoDataset
...
def create_simulation(config: MainConfig) -> Any:
    ...
    eval_seeds = list(getattr(config, "evaluation_seeds", []) or [])
    num_eval_seeds = getattr(config, "num_eval_seeds", len(eval_seeds))
    if num_eval_seeds > 0 and eval_seeds:
        initial_seed = eval_seeds[0]
    else:
        initial_seed = getattr(sim_config, "seed", config.seed)
...
def create_lr_scheduler(optimizer: torch.optim.Optimizer, config: LRSchedulerConfig) -> Any:
    ...
    from RoboMani.utils.lr_schedulers.tri_stage_scheduler import TriStageLRScheduler
