...
class ActionQueryDecoder(nn.Module):
    ...
    def forward(
        ...
        # Allow the decoder to adapt when the number of action tokens varies
        if action_tokens is not None:
            query_len = action_tokens.size(1)
        else:
            query_len = self.query_embed.size(1)

        if query_len > self.query_embed.size(1):
            raise ValueError(
                "ActionQueryDecoder received more action tokens than the number of "
                "learned query embeddings. Increase `action_seq_len` to at least "
                f"{query_len}."
            )

        query = self.query_embed[:, :query_len, :].expand(batch_size, -1, -1)

        if self.use_action_tokens and action_tokens is not None:
            if action_tokens.size(1) != query.size(1):
                action_tokens = action_tokens[:, :query.size(1), :]
...
class RoboManiPolicy(nn.Module):
    def __init__(
            self,
            encoder: Any,
            latent_dim: int,
